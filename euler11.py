...
row10=[21,36,23,9,75,0,76,44,20,45,35,14,00,61,33,97,34,31,33,95]
...
def getMaxProduct(row):
    key = 0
    productList = []
    for length in range(len(row)-3):
        product = 1
        for num in range(4):
            product = product*row[key+num]
...
def maxOfRows():
    maxList = []
    for row in rowList:
        maxList.append(getMaxProduct(row))
    return max(maxList)

#print(maxOfRows())

def createColList(key):
    colList = []
    for row in rowList:
        colList.append(row[key])
    return colList

def maxOfCols():
    maxList = []
    for key in range (20):
        maxList.append(getMaxProduct(createColList(key)))
    return max(maxList)
...
def diagList(x,y):
    diagList = []
    for i in range(4):
        diagList.append(rowList[i+x][i+y])
    product = 1
    for i in range(4):
        product = product * diagList[i]
    return product
...
def diagonalList():
    ...
    for x in range(17):   
        for y in range(17):
            diagonalList.append(diagList(x,y))
    return max(diagonalList)

#print(diagonalList())

def otherDiagList(x,y):
    diagList = []
    for i in range(4):
        diagList.append(rowList[(x+3)-i][i+y])
    product = 1
    for i in range(4):
        product = product * diagList[i]
    return product
    
def otherDiagonalList():
    diagonalList = []
    for x in range(17):   
        for y in range(17):
            diagonalList.append(otherDiagList(x,y))
    return max(diagonalList)        
        
#print(otherDiagonalList())

def main():
    maxList = [maxOfRows(),maxOfCols(),diagonalList(),otherDiagonalList()]
    print(max(maxList))
